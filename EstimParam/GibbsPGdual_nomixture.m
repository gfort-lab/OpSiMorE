function [output] = GibbsPGdual_nomixture(data,MCMC)
%% The   Gibbs PGdual   algorithm
%%
%% for the target distribution pi
...
% Parameters of the target distribution pi
LambdaR = data.LambdaR; % 1 x 1
LambdaO = data.LambdaO; % 1 x 1
...
LogPicurrent = Z'*log(Intensitycurrent)-sum(Intensitycurrent)-LambdaR*StatRcurrent-LambdaO*StatOcurrent;
...
StoreLogPi(1,1) = LogPicurrent;
StoreRchain = zeros(T,NbrMC+1);
StoreRchain(:,1) = Rcurrent;
StoreOchain = zeros(T,NbrMC+1);
StoreOchain(:,1) = Ocurrent;
StoreStatR = zeros(1,NbrMC+1);  
StoreStatR(1,1) = StatRcurrent;
StoreStatO = zeros(1,NbrMC+1);  
StoreStatO(1,1) = StatOcurrent;
...
for nn=1:NbrMC
    ...
    Drift = (max(abs(GdtStep+shift)-GammaTildeR*LambdaR,0).*sign(GdtStep+shift)-shift); % T x 1
    ...
    if test==T
        ...
        Driftrev = (max(abs(GdtSteprev+shift)-GammaTildeR*LambdaR,0).*sign(GdtSteprev+shift)-shift); % T x 1
        ...
        LogPiproposal = Z'*log(Intensityproposal)-sum(Intensityproposal)-LambdaR*StatRproposal-LambdaO*StatOcurrent;
    ...
    Drift = max(abs(GdtStep)-GammaO*LambdaO,0).*sign(GdtStep);  % T x 1
    ...
    if test==T
        ...
        Driftrev = max(abs(GdtSteprev)-GammaO*LambdaO,0).*sign(GdtSteprev);  % T x 1
        NumLogGauss = -sum((Driftrev-Ocurrent).^2)/(4*GammaO);
        DenomLogGauss = -sum((Drift-Oproposal).^2)/(4*GammaO);
        StatOproposal = sum(abs(Oproposal));
        LogPiproposal = Z'*log(Intensityproposal)-sum(Intensityproposal)-LambdaR*StatRcurrent-LambdaO*StatOproposal;
        ARlogratio = LogPiproposal-LogPicurrent+NumLogGauss-DenomLogGauss; % 1 x 1
    ...
    %% Adapt GammaX or compute some statistics, store quantities
    ...
    StoreStatR(1,nn+1) = StatRcurrent;
    StoreStatO(1,nn+1) = StatOcurrent;
    % store the chain
    StoreRchain(:,nn+1) = Rcurrent;
    StoreOchain(:,nn+1) = Ocurrent;
    

    %% Store some quantities of interest
    StoreLogPi(1,nn+1) = LogPicurrent;
end
