function [output] = FullBayesian_PriorGamma_rescaled(data,MCMC)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
samp = MCMC.samp ;     nnn = 0 ;
nu = data.rescaled ; % PA, GF 20250704 Nantes add rescaled Poisson 
...
LogPicurrent = Z'/nu*log(Intensitycurrent)-sum(Intensitycurrent)/nu-LambdaRcurrent*(betaR+StatRcurrent)-LambdaOcurrent*(betaO+StatOcurrent)+StatLRcurrent+StatLOcurrent; % 1 x 1
LogMarginalCurrent = Z'/nu*log(Intensitycurrent)-sum(Intensitycurrent)/nu-TshiftR*log(betaR+StatRcurrent)-TshiftO*log(betaO+StatOcurrent);
...
StoreRchain = zeros(T,NbrMC/samp+1);
StoreOchain = zeros(T,NbrMC/samp+1);
StoreLRchain = zeros(1,NbrMC/samp+1);
StoreLOchain = zeros(1,NbrMC/samp+1);
StorelogPi = zeros(1,NbrMC/samp+1);
StorelogMarginal = zeros(1,NbrMC/samp+1);
...
for nn=1:NbrMC
    ...
    GdtStep = TildeRcurrent+GammaTildeR*invDt*((Z./Intensitycurrent-1).*Phi)/nu ;   % T x 1
    ...
    if test==T
        ...
        GdtSteprev = TildeRproposal+GammaTildeR*invDt*((Z./Intensityproposal-1).*Phi)/nu ;   % T x 1
        ...
        LogPiproposal = Z'/nu*log(Intensityproposal)-sum(Intensityproposal)/nu-LambdaRcurrent*(betaR+StatRproposal)-LambdaOcurrent*(betaO+StatOcurrent)+StatLRcurrent+StatLOcurrent; % 1 x 1
    ...
    GdtStep = Ocurrent+GammaO*(Z./Intensitycurrent-1)/nu;   % T x 1
    ...
    if test==T
        ...
        GdtSteprev = Oproposal+GammaO*(Z./Intensityproposal-1)/nu ;   % T x 1
        ...
        LogPiproposal = Z'/nu*log(Intensityproposal)-sum(Intensityproposal)/nu -LambdaRcurrent*(betaR+StatRcurrent)-LambdaOcurrent*(betaO+StatOproposal)+StatLRcurrent+StatLOcurrent; % 1 x 1
    ...
    auxLog =  Z'/nu*log(Intensitycurrent)-sum(Intensitycurrent)/nu;   % 1 x 1
...
%         StoreLOchain(1,nn+1) = LambdaOcurrent; % 1 x 1
        if ~mod(nn,samp)
            nnn = nnn + 1 ;
            StorelogPi(:,nnn+1) = LogPicurrent; % 1 x 1
            StorelogMarginal(:,nnn+1) = LogMarginalCurrent; % 1 x 1
            StoreRchain(:,nnn+1) = Rcurrent; % T x 1
            StoreOchain(:,nnn+1) = Ocurrent; % T X 1
            StoreLRchain(1,nnn+1) = LambdaRcurrent; % 1 x 1
            StoreLOchain(1,nnn+1) = LambdaOcurrent; % 1 x 1
        end
end
...
auxRchain = StoreRchain(:,burnin/samp+1:NbrMC/samp+1); % T x ...
auxOchain = StoreOchain(:,burnin/samp+1:NbrMC/samp+1); % T x ...
auxLRchain = StoreLRchain(1,burnin/samp+1:NbrMC/samp+1); % 1 x ...
auxLOchain = StoreLOchain(1,burnin/samp+1:NbrMC/samp+1); % 1 x ...
